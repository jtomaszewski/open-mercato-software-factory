...
const logger = createLogger('factory').child({ interceptor: 'order-fulfilled' })
 ...
 * transition emits `factory.order.fulfilled`. The intake itself runs in a persistent subscriber,
 * so the salesperson's save never waits on or fails with the factory.
 */
const orderFulfilled: CommandInterceptor = {
  id: 'factory.order-fulfilled',
  ...
  async afterExecute(input, _result, context) {
    ...
    try {
      ...
      await emitFactoryEvent('factory.order.fulfilled', { id: orderId, ...scope }, { persistent: true, ...scope })
    } catch (error) {
