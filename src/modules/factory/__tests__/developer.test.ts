...
import { developerBranch, openDeveloperPullRequest, pullRequestSummary } from '../lib/developer'
...
it('commits the agent’s files on the exact base it was given and opens one PR per task', async () => {
  const result = await openDeveloperPullRequest(deps, task, change)
  ...
  expect(result).toEqual({ prNumber: 9, prUrl: 'https://github.com/o/site/pull/9', prLabel: `PR #9 · ${task.title}`, branch: developerBranch(task.id) })
})

it('falls back to a generic description when the agent gave no summary', async () => {
  await openDeveloperPullRequest(deps, task, { ...change, summary: '' })
  expect(github.createPullRequest.mock.calls[0]![0].body).toContain('Zmiana przygotowana przez agenta Developer.')
})
