...
import aiTools, {
  FACTORY_REQUEST_CHANGE_FEATURES,
  FACTORY_REQUEST_CHANGE_TOOL,
  requestChangeInputSchema,
} from '../ai-tools'

function factoryFake(options: { includeAgent?: boolean } = {}) {
...
function tool(): AiToolDefinition {
  const found = aiTools.find((entry) => entry.name === FACTORY_REQUEST_CHANGE_TOOL)
  if (!found) throw new Error('factory.request_change tool missing')
  return found
}
...
function context(overrides: Partial<McpToolContext> = {}): McpToolContext {
  return {
    ...
    userFeatures: [...FACTORY_REQUEST_CHANGE_FEATURES],
    isSuperAdmin: false,
    ...overrides,
  }
}
...
describe('factory.request_change registry contract', () => {
...
describe('factory.request_change execution', () => {
  ...
  it('refuses missing scope or Developer before creating a task', async () => {
    ...
    fake = factoryFake({ includeAgent: false })
    await expect(tool().handler(
      { title: 'Zmień stronę', instructions: 'Zmień stopkę.' },
      context(),
