...
export const FACTORY_REQUEST_CHANGE_TOOL = 'factory.request_change'
export const FACTORY_REQUEST_CHANGE_FEATURES = [
...
type FactoryToolScope = { tenantId: string; organizationId: string; userId: string }

function requireFactoryToolScope(context: Pick<McpToolContext, 'tenantId' | 'organizationId' | 'userId'>): FactoryToolScope {
  ...
  if (!tenantId || !organizationId || !userId) {
    throw new Error('[internal] factory.request_change requires a tenant-, organization-, and user-scoped context')
  }
...
function apiFor(context: McpToolContext, tool: AiToolDefinition): { staff: StaffApi; runner: AiApiOperationRunner } {
  requireFactoryToolScope(context)
