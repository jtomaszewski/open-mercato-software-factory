...
import { DEFAULT_REPO, readEnv } from './github'

/**
 * The repository checkout the coding agent works on. It lives under the orchestrator's OpenCode
 * sandbox root (the one bind-mounted into the sidecar), at `tasks/<taskId>`, so the file agent
...
/** The sandbox subdirectory the task checkouts live in. */
const CHECKOUT_DIR = 'tasks'
...
export function readCheckoutConfigFromEnv(env: NodeJS.ProcessEnv = process.env, cwd: string = process.cwd()): CheckoutConfig {
  const repo = readEnv(env, 'REPO', 'SITE_REPO') || DEFAULT_REPO
  if (!/^[\w.-]+\/[\w.-]+$/.test(repo)) throw new CheckoutError(`CODE_CHANGES_REPO must be owner/name, got "${repo}"`, 'setup')
  const workspaceRoot = env.OM_OPENCODE_WORKSPACE_ROOT?.trim() || './.mercato/opencode-work'
  return {
    repo,
    baseBranch: readEnv(env, 'BASE_BRANCH', 'SITE_BASE_BRANCH') || 'main',
    ...
    gitRoot: resolve(cwd, readEnv(env, 'CHECKOUT_GIT_ROOT', 'CHECKOUT_GIT_ROOT') || './.mercato/code-changes-git'),
  }
}

/** Paths the agent must never change (execution spec: protected CI/provider controls). */
...
export function checkoutPaths(config: CheckoutConfig, taskId: string): { work: string; gitDir: string; workDir: string } {
  ...
  return {
    work: join(config.workspaceRoot, CHECKOUT_DIR, id),
    gitDir: join(config.gitRoot, id),
    workDir: `${config.containerWorkspaceRoot}/${CHECKOUT_DIR}/${id}`,
  }
}
...
export async function prepareCheckout(config: CheckoutConfig, taskId: string, exec: Exec = execCommand, token?: string): Promise<PreparedCheckout> {
  ...
  await mkdir(join(config.workspaceRoot, CHECKOUT_DIR), { recursive: true })
...
export async function collectChanges(config: CheckoutConfig, taskId: string, exec: Exec = execCommand): Promise<CollectedChange> {
  ...
  if (changes.length === 0) throw new CheckoutError('The agent finished without changing any file.', 'no_changes')
  const blocked = protectedPaths(changes.map((change) => change.path))
  if (blocked.length) throw new CheckoutError(`The agent changed protected paths: ${blocked.join(', ')}`, 'protected_path')
  ...
  for (const change of changes) {
    ...
    if (!absolute.startsWith(`${paths.work}/`) || (await lstat(absolute)).isSymbolicLink()) {
      throw new CheckoutError(`The agent produced a link or path outside the repository: ${change.path}`, 'protected_path')
    }
