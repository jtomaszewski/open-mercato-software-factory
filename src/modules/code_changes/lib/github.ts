/**
 * The few GitHub REST calls a delegated code change needs to open one PR with a handful of files:
...
export const DEFAULT_REPO = 'jtomaszewski/hackaton-stal-zbiorniki-landing'

/** `CODE_CHANGES_<name>`, else its pre-rename `FACTORY_<legacy>` name still in older `.env` files. */
export function readEnv(env: NodeJS.ProcessEnv, name: string, legacy: string): string | undefined {
  return env[`CODE_CHANGES_${name}`]?.trim() || env[`FACTORY_${legacy}`]?.trim() || undefined
}

/** The repository a task project without a registered repository works on. */
export function readGitHubConfigFromEnv(env: NodeJS.ProcessEnv = process.env): GitHubConfig {
  const token = readEnv(env, 'GITHUB_TOKEN', 'GITHUB_TOKEN')
  if (!token) throw new Error('CODE_CHANGES_GITHUB_TOKEN is not set and the project has no linked repository; cannot open pull requests.')
  const repo = readEnv(env, 'REPO', 'SITE_REPO') || DEFAULT_REPO
  if (!/^[\w.-]+\/[\w.-]+$/.test(repo)) throw new Error(`CODE_CHANGES_REPO must be owner/name, got "${repo}"`)
  return {
    token,
    repo,
