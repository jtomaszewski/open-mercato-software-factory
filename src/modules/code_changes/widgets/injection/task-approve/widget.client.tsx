...
function FilePatch({ file }: { file: ReviewFile }) {
  const t = useT()
  return <details className="rounded-md border border-border" data-testid="code-changes-review-file">
...
export default function TaskApprove({ context }: { context?: { taskId?: string } }) {
  ...
  React.useEffect(() => {
    ...
    setLoading(true)
    setLoadError(false)
    void readApiResultOrThrow<{ review: TaskReview | null }>(`/api/code_changes/tasks/${taskId}/review`)
  ...
  async function approve() {
    ...
    setError(null)
    try {
      await apiCallOrThrow(`/api/code_changes/tasks/${review.taskId}/approve`, { method: 'POST' })
      refresh()
      ...
      setError(failure instanceof Error && failure.message ? failure.message : t('code_changes.approve.errors.generic'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const checksLabel = failing.length ? t('code_changes.review.checks.failing') : pending.length ? t('code_changes.review.checks.pending') : review.checks.length ? t('code_changes.review.checks.passing') : t('code_changes.review.checks.none')
  ...
  return <section className="space-y-2" aria-label={t('code_changes.approve.title')} data-testid="code-changes-task-approve">
    ...
    <details className="rounded-md border border-border px-3 py-2" data-testid="code-changes-review-technical">
      <summary className="cursor-pointer text-xs font-medium text-muted-foreground">{t('code_changes.review.technicalDetails', 'Pull request and changed files')}</summary>
      <div className="mt-2 space-y-1">
        <a className="text-sm text-primary underline" href={review.pr.url} target="_blank" rel="noopener noreferrer">{t('code_changes.review.pr', 'PR #{number}').replace('{number}', String(review.pr.number))}</a>
