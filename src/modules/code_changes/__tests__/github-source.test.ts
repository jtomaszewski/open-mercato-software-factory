...
import { resolveTaskGitHub } from '../lib/github-source'

const scope = { tenantId: 'tenant-1', organizationId: 'org-1' }
const env = { CODE_CHANGES_GITHUB_TOKEN: 'env-token', CODE_CHANGES_REPO: 'env/site', CODE_CHANGES_BASE_BRANCH: 'main' } as unknown as NodeJS.ProcessEnv
const legacyEnv = { FACTORY_GITHUB_TOKEN: 'env-token', FACTORY_SITE_REPO: 'env/site', FACTORY_SITE_BASE_BRANCH: 'main' } as unknown as NodeJS.ProcessEnv
...
it('falls back to the env repo and token when the project has no linked repository', async () => {
  ...
  await expect(resolveTaskGitHub(container(), scope, 'project-1', legacyEnv)).resolves.toEqual(expected)
})
...
it('does not fall back when a linked repository cannot be used', async () => {
  ...
  await expect(resolveTaskGitHub(container(forProject), scope, 'project-1', env)).rejects.toThrow('repositoryUnavailable')
})
