...
describe('checkout helpers', () => {
  ...
  it('puts the checkout under the sandbox root and its git dir outside it', () => {
    ...
    expect(config).toMatchObject({ workspaceRoot: '/app/.mercato/opencode-work', containerWorkspaceRoot: '/home/opencode/work', gitRoot: '/app/.mercato/code-changes-git' })
    expect(checkoutPaths(config, 'task-1')).toEqual({
      work: '/app/.mercato/opencode-work/tasks/task-1', gitDir: '/app/.mercato/code-changes-git/task-1', workDir: '/home/opencode/work/tasks/task-1',
    })
    expect(() => checkoutPaths(config, '../x')).toThrow('Refusing')
    expect(() => readCheckoutConfigFromEnv({ CODE_CHANGES_REPO: 'nope' } as unknown as NodeJS.ProcessEnv)).toThrow('owner/name')
  })
})
...
describe('prepareCheckout + collectChanges', () => {
  ...
  it('clones into the sandbox, keeps .git out of it, and reads the changed source files back', async () => {
    ...
    expect(prepared).toEqual({ baseSha: 'base-sha', workDir: '/home/opencode/work/tasks/task-1' })
    const clone = calls.find((call) => call.args[0] === 'clone')!
    expect(clone.args).toEqual(expect.arrayContaining([`--separate-git-dir=${join(root, 'git/task-1')}`, 'https://github.com/o/site.git', join(root, 'work/tasks/task-1')]))
    ...
    await mkdir(join(root, 'work/tasks/task-1/app'), { recursive: true })
    await writeFile(join(root, 'work/tasks/task-1/app/new.tsx'), 'export default 1\n')
  ...
  it('refuses a protected path, an empty change and a symlink out of the checkout', async () => {
    ...
    await symlink('/etc/hosts', join(root, 'work/tasks/task-1/leak.txt'))
    await expect(collectChanges(config, 'task-1', exec)).rejects.toMatchObject({ reason: 'protected_path' })
  })
