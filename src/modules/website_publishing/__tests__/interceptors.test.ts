...
jest.mock('../events', () => ({ emitWebsitePublishingEvent: (...args: unknown[]) => emit(...args) }))
...
it('emits website_publishing.order.fulfilled, scoped from the caller, only on the switch to fulfilled', async () => {
  ...
  expect(emit).toHaveBeenCalledWith('website_publishing.order.fulfilled', { id: 'order-1', ...scope }, { persistent: true, ...scope })
})
