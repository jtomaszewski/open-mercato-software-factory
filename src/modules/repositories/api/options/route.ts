...
export async function GET(request: Request) {
  return withRepositoryRoute(request, ['repositories.view'], async ({ commandContext, tenantId, organizationId }) => {
    ...
    return Response.json({ items: rows.map((repository) => ({ id: repository.id, fullName: repository.fullName, status: repository.status })), totalCount })
  })
}

export const openApi: OpenApiRouteDoc = {
  tag: 'Repositories', summary: 'Repository options', methods: {
    GET: { summary: 'List repositories available for project linking', responses: [{ status: 200, description: 'Repository options', schema: z.object({ items: z.array(z.object({ id: z.string().uuid(), fullName: z.string(), status: z.enum(['active', 'disabled']) })), totalCount: z.number().int() }) }] },
  },
}
