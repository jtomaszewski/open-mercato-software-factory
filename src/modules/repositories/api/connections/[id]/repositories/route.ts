...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  return withRepositoryRoute(request, ['repositories.view'], async ({ commandContext, tenantId, organizationId }) => {
    ...
    const grant = await commandContext.container.resolve<RepositoryBroker>(REPOSITORY_BROKER).refreshInstallation({ installationId: connection.installationId, authorizationId: connection.brokerAuthorizationId })
...
export const openApi: OpenApiRouteDoc = { tag: repositoriesTag, summary: 'Read current GitHub grants', methods: {
  GET: { summary: 'Refresh repositories granted to a connected installation', responses: [{ status: 200, description: 'Current grant', schema: installationGrantSchema }], errors: [{ status: 404, description: 'Connection not found', schema: repositoryErrorSchema }, { status: 502, description: 'Broker unavailable', schema: repositoryErrorSchema }] },
} }
