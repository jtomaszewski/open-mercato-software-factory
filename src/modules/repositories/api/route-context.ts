...
import { RepositoryBrokerError } from '../lib/broker'
...
export async function withRepositoryRoute(
  ...
  try {
    ...
    if (error instanceof RepositoryBrokerError) {
      const status = error.code === 'not_configured' || error.code === 'invalid_configuration' ? 503 : 502
      return Response.json({ error: `repositories.errors.broker.${error.code}` }, { status })
    }
    throw error
  }
}
