...
export async function GET(request: Request) {
  return withRepositoryRoute(request, ['repositories.link'], async (context) => {
    ...
    const items = await Promise.all(links.map(async (link) => {
      ...
      return repository ? { id: link.id, projectId, repositoryId: repository.id, fullName: repository.fullName, status: repository.status, isDefault: link.isDefault, updatedAt: link.updatedAt.toISOString() } : null
    }))
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Repositories', summary: 'Project repository links', methods: {
    GET: { summary: 'List repository links for a staff project', responses: [{ status: 200, description: 'Project repository links', schema: z.object({ items: z.array(projectLinkResponseSchema.extend({ fullName: z.string(), status: z.enum(['active', 'disabled']) })) }) }], errors: [{ status: 403, description: 'Missing project access', schema: errorSchema }, { status: 404, description: 'Project not found', schema: errorSchema }] },
