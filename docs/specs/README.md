...
| [006](./SPEC-006-2026-09-19-factory-tools.md) | 2026-09-19 | Factory tools: chat intake and site drift detection as AI tools | Draft | One app module `factory_tools` with a chat intake agent and six `defineAiTool` tools served by the Open Mercato chat (approval card) and its MCP server: create/read/search/comment on `staff` tasks with optional delegation through `tasks`, and a read-only drift check comparing the live site's `data-*` attributes with the catalog, each drift repairable as one task; the chat writes only tasks (ADR-0002) |
