...
| 2026-09-19 | MCP intake tool `factory_send_task` superseded by `task_tools.create_task` (SPEC-006): one write tool served by the Open Mercato MCP server, `{ project, title, description, delegate?, agentUserId?, links?, clientRef? }`; source recorded as a task footer until the `tasks_intake` table exists. |
