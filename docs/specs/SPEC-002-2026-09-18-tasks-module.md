...
| 2026-09-19 | Intake tool moved to SPEC-006 as the `task_tools` module, served over the Open Mercato MCP server (`task_tools.create_task` replacing `tasks_create`); no in-app chat agent. It creates through `staff`'s task route and delegates through this module's `GET /api/tasks/agents` + `POST /api/tasks/delegations` when installed. `source_ref = '{conversationId}:{messageId}'` is not obtainable in 0.8.0 (handlers get no conversation or message id), so tasks record the client's `clientRef` in a footer; the `tasks_intake` table stays this module's roadmap. |
